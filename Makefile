...
pr_check:
	for changed_yaml in `git diff remotes/origin/main --name-only | grep .yaml$$ | grep -v '.shema.yaml'`; do python scripts/pr-check.py $${changed_yaml} && pykwalify -d $${changed_yaml} -s .schema.yaml ; done
